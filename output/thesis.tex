...
of the power system concepts and phenomena that are particularly
relevant within operational timeframes. It also outlines processes,
services and markets that are relevant to active power balancing in
operational timeframes, with those discussed in detail within this
thesis highlighted in bold red text.
...
this thesis. Timeframes and phenomena were adapted from Machowski et al.
(\protect\hyperlink{ref-machowskiPowerSystemDynamics2020}{2020}) and
stability categories from Hatziargyriou et al.
...
operations is concerned with phenomena that dominate from millisecond

The dynamic behaviour of a power system is primarily determined by its
underlying energy conversion processes and the way in which components
and resources are coupled to a synchronous area. \emph{Power system
dynamics} can be classified based on their physical characteristics and
on the timescales at which they dominate
(\protect\hyperlink{fig:dynamics}{3}). In the context of power system
frequency control, we are concerned with the following classifications
...
\item
  \emph{Electromagnetic} dynamics, which include millisecond-scale
  phenomena that occur due to the coupling of electrical and magnetic
  fields within synchronous machines, within the power electronics
  devices of \emph{inverter-based resources} (IBR), and between
  synchronous machines, IBR and the wider network.
\item
  \emph{Electromechanical} dynamics, which is a slower set of phenomena
  (seconds timescale) that arise as a result of electromagnetic fields
  interacting with rotating masses (e.g.~turbines and motors).
\item
  \emph{Thermodynamic} phenomena, which typically encompass chemical
  fuel conversion processes (e.g.~boilers and their control) in
  conventional power systems. We will extend this definition to include
  the dynamic behaviour of primary energy sources for hydroelectricity
  and \emph{variable renewable energy} (VRE), and the energy conversion
  process for battery energy storage systems (BESS). These phenomena
  occur multiple timeframes, from seconds to minutes to hours.
\end{enumerate}

\begin{figure}
\hypertarget{fig:dynamics}{%
\centering
\includegraphics{source/figures/dynamics.png}
\caption{Timeframes of power system dynamics. Source:
(\protect\hyperlink{ref-Machowski2020}{\textbf{Machowski2020?}}).}\label{fig:dynamics}
}
\end{figure}
